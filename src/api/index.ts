...
export function getVideoTitleFromManifest(manifest: IManifest, lang: string = "en") {
...
export function getVideoPartTitle(videoPart: IVideoPart, lang: string = "en"): string {
...
function getVideoPartAnnotationPageByLabel(videoPart: IVideoPart, matchFunc: (label: string) => boolean): Array<IVTT> {
    return videoPart.annotations.filter(
        annotation => {
            return matchFunc(annotation.label["en"][0])
        }).map(vtt => {
            return {
                items: [],
                ...vtt
...
export function getVideoPartVTTs(videoPart: IVideoPart): Array<IVTT> {
    return getVideoPartAnnotationPageByLabel(
        videoPart,
        label => label.endsWith(".vtt"))
